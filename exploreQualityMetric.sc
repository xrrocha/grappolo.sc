...
val metrics =
  Source.fromFile("metrics.txt").getLines()
...
val (distances, clusterCounts, medoidDistances) =
  metrics
    .foldLeft((List[Double](), List[Double](), List[Double]())) { (accum, nums) =>
      val (distances, clusterCounts, medoidDistances) = accum
      val (distance, clusterCount, medoidDistance) = nums
      (distances :+ distance, clusterCounts :+ clusterCount, medoidDistances :+ medoidDistance)
    }
...
Using(PrintWriter(FileWriter("normalized-metrics.txt"))) { out =>
  metrics.zip(normalizedMetrics)
    .map { (m, n) =>
      ...
      val (normalizedDistance, normalizedClusterCount, normalizedMedoidDistance) = n
      List(
        distance, clusterCount, medoidDistance, quality(m),
        normalizedDistance, normalizedClusterCount, normalizedMedoidDistance, quality(n)
      )
    }
    .sortBy(_(7))
    .foreach { list =>
